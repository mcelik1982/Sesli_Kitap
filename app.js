document.addEventListener('DOMContentLoaded', () => {
    ...
    bookCoverInput.addEventListener('change', (e) => {
        if(e.target.files.length > 0) coverFileName.textContent = e.target.files[0].name;
        else coverFileName.textContent = 'Dosya seçilmedi';
    });

    bookAudioInput.addEventListener('change', (e) => {
        if(e.target.files.length > 0) audioFileName.textContent = e.target.files[0].name;
        else audioFileName.textContent = 'Dosya seçilmedi';
    });
